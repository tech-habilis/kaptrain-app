...
import {
  TAuthUser,
  TPayloadResendSignUpVerificationMail,
  TPayloadResetPassword,
  TPayloadSendForgotPasswordMail,
  TPayloadSignIn,
  TPayloadSignUp,
  TPayloadVerifyOtp,
...
export async function signInWithOtpSupabase(payload: {
  email: string
}): Promise<void> {
...
export async function signInWithPasswordSupabase(
  ...
  if (user.profilePicture)
    user.profilePicture = new FileHandler().readFile({
...
): Promise<{ user: TAuthUser | null; session: Session | null }> {
