...
export function useAgendaCalendar(userId: string | undefined): UseAgendaCalendarReturn {
  ...
  const currentMonthLabel = currentDate.format("MMMM YYYY").replace(/^\w/, (c) => c.toUpperCase());
  ...
  // Fetch sessions when currentDate or userId changes
  /* eslint-disable react-hooks/exhaustive-deps */
  useEffect(() => {
    fetchSessions();
  }, [currentDate, userId]); // fetchSessions is intentionally omitted to prevent infinite loop
  ...
  const calendarWeeks = useMemo<CalendarDay[][]>(() => {
    ...
    const calendarStart = monthStart.subtract(firstDayOfWeek === 0 ? 6 : firstDayOfWeek - 1, "day");

    // Build a map of sessions by date for activity dots
    const sessionsByDate = new Map<string, ("blue" | "orange" | "grey" | "green")[]>();
...
function getActivityColor(session: Session): "blue" | "orange" | "grey" | "green" {
